...
import { Search, User, Bell, UserPlus } from 'lucide-react';
...
const Employees = () => {
  ...
  // ✅ Fetch attendants dynamically
  useEffect(() => {
    const fetchAttendants = async () => {
      try {
        const token = localStorage.getItem("token");
        const res = await axios.get("http://localhost:3000/attendant", {
  ...
  const filteredData = employees.filter(
    (employee) =>
      ...
      (employee.substation?.name?.toLowerCase().includes(searchTerm.toLowerCase()))
  );
            ...
            <div className="flex justify-between items-center mb-4 flex-col sm:flex-row gap-4">
              <div className="relative w-full sm:w-64">
                ...
                <Input
                  ...
                  className="bg-[#201E1D] flex h-10 rounded-md px-3 py-2 focus-visible:outline-none focus-visible:ring-2 border-none focus-visible:ring-[#5EC9ED] pl-8"
                  value={searchTerm}
                    ...
                    {currentItems.map((employee) => (
                      <TableRow key={employee._id}>
                        ...
                        <TableCell>
                          {employee.substation ? (
                            <button
                              onClick={() => handleSubstationClick(employee.substation.name)}
